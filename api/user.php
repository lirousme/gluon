...
function ensureGraphSystemDeck(PDO $pdo, int $user_id): int {
    ...
    try {
        ...
        $stmtRootOrder = $pdo->prepare("SELECT COALESCE(MAX(sort_order), -1) + 1 FROM directories WHERE user_id = ? AND parent_id IS NULL FOR UPDATE");
        $stmtRootOrder->execute([$user_id]);
        $nextSortOrder = (int)$stmtRootOrder->fetchColumn();
...
if ($action === 'get_session') {
    ensureGraphSystemDeck($pdo, $user_id);
