...
elseif ($action === 'add_grammar_entry') {
    ...
    $supportedTables = ['nouns', 'verbs'];

    if (!in_array($table, $supportedTables, true)) {
        die(json_encode(['status' => 'error', 'message' => 'No momento, apenas substantivos e verbos podem ser salvos por este formulário.']));
    }

    try {
        $stmtColumns = $pdo->prepare("SELECT COLUMN_NAME, IS_NULLABLE, COLUMN_DEFAULT, EXTRA FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = ? ORDER BY ORDINAL_POSITION");
        $stmtColumns->execute([$table]);
        $columnRows = $stmtColumns->fetchAll(PDO::FETCH_ASSOC);

        if (!$columnRows) {
            throw new RuntimeException('Tabela não encontrada: ' . $table);
        }

        $columns = ['id_user'];
        $values = [$user_id];

        foreach ($columnRows as $columnInfo) {
            $column = (string)($columnInfo['COLUMN_NAME'] ?? '');
            if ($column === '' || $column === 'id' || $column === 'id_user' || $column === 'created_at' || $column === 'updated_at') {
                continue;
            }
